...
export function ShiftClosingBillSettings() {
  ...
  useEffect(() => {
    api
      ...
      .then((v: any) => {
        if (v && typeof v === 'object') setTemplate({ ...DEFAULT_SHIFT_CLOSING_BILL_TEMPLATE, ...v });
      })
      .catch(() => {})
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.saveSetting(SETTING_KEY, template);
      setSaved(true);
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Tùy chỉnh nội dung bill in khi nhân viên kết ca</p>
          </div>
        </div>
        <button
          onClick={handleSave}

...
export function CheckoutPanel({ cart, branchId, currentShifts = [], onRemoveItem, onUpdateQuantity, onClearCart, onAddItem, comboMode = false }: CheckoutPanelProps) {
  ...
  // Chế độ combo: chọn giao hàng / nhận tại quầy (+ địa chỉ nếu giao)
  ...
  const completePayment = async () => {
    ...
    const orderItems = cart.map(item => ({
      ...
      toppings: item.toppings,
      isCustomCombo: item.isCustomCombo,
      // Chế độ combo: đánh dấu để bill kết ca + lịch sử hiện là đơn combo.
  ...
  return (
    <div className="pos-checkout flex flex-col h-full w-full bg-white rounded-lg shadow-md relative border-2 border-gray-100">
      ...
      {staffOptions.length > 1 && (
        <div className="shrink-0 flex items-center gap-2 px-3 py-2 bg-amber-50 border-b border-amber-200">
          ...
          <select
            ...
            className="flex-1 min-w-0 border border-amber-300 rounded-lg px-2 py-1 text-sm font-bold text-gray-800 bg-white"
          >
            {staffOptions.map((s) => (
      ...
      {comboMode && checkoutStep === 'cart' && (
        <div className="shrink-0 px-3 py-2 bg-indigo-50 border-b border-indigo-200 space-y-2">
          <div className="text-xs font-black text-indigo-700">🎁 ĐƠN COMBO</div>
          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={() => setComboDeliveryType('pickup')}
              className={`py-2 rounded-lg text-sm font-bold border ${comboDeliveryType === 'pickup' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-300'}`}
            >
              🏬 Tại quầy
            </button>
            <button
              type="button"
              onClick={() => setComboDeliveryType('delivery')}
              className={`py-2 rounded-lg text-sm font-bold border ${comboDeliveryType === 'delivery' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-300'}`}
            >
              🛵 Giao hàng
            </button>
          </div>
          {comboDeliveryType === 'delivery' && (
            <input
              value={comboAddress}
              onChange={(e) => setComboAddress(e.target.value)}
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="pos-cart-btn-primary bg-amber-600 active:bg-amber-700 text-white rounded-xl flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                className="pos-cart-btn-primary bg-gray-700 active:bg-gray-800 text-white rounded-xl flex items-center justify-center gap-2"
              >
                <Printer className="w-5 h-5" />
                Bill khách
              </button>

...
export async function fetchOrders(params?: { branchId?: string; salesStaffId?: string; shiftId?: string; recentDays?: number }) {
  ...
  if (params?.recentDays) qs.set('recentDays', String(params.recentDays));
...
export async function rescheduleDeliveryLog(
  id: string,
  body: { deliveryDate?: string; deliveryTime?: string; deliveryAddress?: string; branchId?: string; note?: string; productName?: string; productId?: string; size?: string; protein?: number; deliveryType?: string; shipMethod?: string; shipProvider?: string }
) {
